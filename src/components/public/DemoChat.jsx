...
export default function DemoChat({ t, lang, scenario, messages, setMessages, onAgentResponse, pendingExample, clearPendingExample, winery, experiences }) {
  ...
  const sendMessage = async (text) => {
    ...
    const payload = {
      ...
      winery: winery || { name: "Celler Demo" },
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { role: "assistant", content: t.chatError || "Error de connexió." },
      ]);
    } finally {
      setLoading(false);
    }
    ...
    <div className="flex flex-col h-full bg-white rounded-2xl border border-stone-200 shadow-lg overflow-hidden">
      <div className="px-5 py-3.5 border-b border-stone-100 bg-gradient-to-r from-[#FAF7F2] to-white">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <p className="text-sm font-semibold text-[#2D1B14]">{winery?.nombre || "Celler Demo"}</p>
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-3 min-h-[400px] max-h-[500px]">
        {messages.map((msg, i) => (
