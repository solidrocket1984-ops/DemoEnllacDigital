...
export default function DemoHero({ t, onDemoClick, winery }) {
  const wineryName = winery?.nombre || winery?.name || "Celler Demo";
      ...
      <div className="relative max-w-5xl mx-auto px-4 py-16 sm:py-24">
        ...
        >
          ...
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/10 backdrop-blur-sm border border-white/20 text-white/80 text-xs font-medium mb-8">
            <Wine className="w-3.5 h-3.5 text-[#C9A962]" />
            <span className="text-[#C9A962] font-semibold">{wineryName}</span>
            <span className="text-white/40 mx-1">·</span>
            <span>Demo de l'assistent intel·ligent</span>
          </div>

          <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold text-white leading-tight mb-6 tracking-tight">
            {t.heroTitle}
          </h1>
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <button
              onClick={onDemoClick}
              className="inline-flex items-center justify-center gap-2 px-8 py-3.5 rounded-full bg-[#722F37] text-white font-medium text-sm hover:bg-[#5C252D] transition-all shadow-lg shadow-[#722F37]/40"
            >
              {t.heroBtn1}
          ...
          <div className="mt-12 flex flex-wrap justify-center gap-6 text-xs text-white/40">
            {["Conversa real", "Detecció d'intenció", "Captació de lead", "Guardado a BBDD"].map((step, i) => (
              <div key={i} className="flex items-center gap-2">
                <span className="w-5 h-5 rounded-full bg-white/10 flex items-center justify-center text-[10px] font-bold text-white/60">{i + 1}</span>
                <span>{step}</span>
