        ...
        {isLoading ? (
          ...
          <div className="grid gap-3">
            {filtered.map((lead) => (
              <div
                key={lead.id}
                className="bg-white border border-slate-200 rounded-xl p-5 hover:shadow-md transition-all cursor-pointer"
                onClick={() => setSelectedLead(lead)}
              >
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-wrap items-center gap-2 mb-2">
                      <span className="font-semibold text-slate-900">{lead.wineryName || "Demo"}</span>
                      {lead.leadStage && (
                      ...
                      {lead.exportStatus === "exported" && (
                        <span className="px-2 py-0.5 rounded-full text-[10px] font-medium bg-purple-100 text-purple-700">Exportat</span>
                      )}
                    ...
                    <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-slate-500">
                      ...
                      {lead.language && (
                        <span className="uppercase font-mono text-[10px] bg-stone-100 px-1.5 py-0.5 rounded">
                          {lead.language}
                        </span>
                      )}
                    </div>
                    ...
                    {lead.conversationSummary && (
                      <p className="text-xs text-slate-400 mt-2 line-clamp-1">{lead.conversationSummary}</p>
                    )}
                  </div>
